...
class S3Filesystem implements AdapterInterface
{
	...
	public function createFile(string $name, string $path, $data): string
	{
		...
		//Solução para o erro de mime type de arquivos, comentar o arquivo de headers acima e cirar um novo parametro em Connector putObject, mimeType. #melhoria
		$fileType = pathinfo($name, PATHINFO_EXTENSION) ?? 'bin';
		$mimeType = self::MIME_TYPES[strtolower($fileType)];
		...
		$this->connector->putObject($input, $this->bucket, $directory . $name, Acl::ACL_PUBLIC_READ, $headers, $mimeType);
