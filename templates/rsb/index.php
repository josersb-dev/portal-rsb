...
<html lang="<?php echo $this->language; ?>" dir="<?php echo $this->direction; ?>">
	<head>
		...
		$doc->addScript(JPATH_THEMES.'/'.$template->template.'/js/portal.js?'.uniqid());
		$theme->add_scss('master', $scssVars, 'template');

		$theme->add_css('padrao-rsb.css');
		//$theme->add_css('portal.css');

		//$doc->addStyleSheet(Juri::base(true).'/templates/'.$template->template.'/css/padrao-rsb.css?'.uniqid());
		$doc->addStyleSheet(Juri::base(true).'/templates/'.$template->template.'/css/portal.css?'.uniqid());
	...
	<body class="<?php echo $theme->bodyClass(); ?>">
		...
		<script>
			jQuery(function($){
				$(document).on('click','.rsb-search-menu',function(e){
					e.preventDefault();
					let $el = $(this)
					$(this).toggleClass('active').promise().done(function(){
						if($el.find('.js-finder-search-query').css('display') == 'none'){
							$el.find('.js-finder-search-query').fadeIn()
						}else{
							$el.find('.js-finder-search-query').fadeOut()
						}
					})
					setTimeout(() => {
						$el.find('.js-finder-search-query').focus()
					}, 100);
					
				})

				$(document).on('click','.rsb-search-menu input',function(e){
					e.stopPropagation();
				})

				let formNewsLetter = $('#awForm-138');
				formNewsLetter.find('.checkbox-inline').append(`<p class="p-body " style="color: #a0a0a0">
                        O uso de seus dados é regido pela nossa
                        <a href="/portal-de-privacidade" style="" target="_blank">
                           Política de Privacidade
                        </a>
                     </p>`)

					$('[href="/component/content/category?layout=blog&amp;Itemid=101"]').attr('href','nadabb')
			})
		</script>
	</body>
</html>