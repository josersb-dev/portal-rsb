...
class awRender {
	public static function getDados($json,$moduleId,$dbName = null,&$params = null){
		...
		{
			if(!awRenderEdit::getItem('awToken',$dbName))
			{	
...
public static function render($fields,$wColumn,$captcha,$moduleId,$params){
	...
	********************************************/
	...
	$label 		= !empty($config['label']) && $tag != 'button' && $tag != 'a' ? '<label for="'.$name.'" class="aw-form-label">'.$config['label'].$awRequired.'</label>' : null;
	$valid = !empty($valid) ? 'valid="true" autocomplete="off" required' : null;
	...
	****************/
	...
	switch ($tag) {
    	...
    	case 'h1':
    		...
    		$renderFields[] = '<'.$hRt.$attr.'>'.$content.'</'.$hRt.'>';
    		break;
		case 'p':
			 $renderForm[] = html_entity_decode($content);
			break;
		default:
			# code...
			break;
