...
class ItemModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_ACERVOSALESIANO';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_acervosalesiano.item';
	...
	*/
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_acervosalesiano.edit.item.data', array());
		...
		if (empty($data))
		{
			...
			// Support for multiple or not foreign key field: etapa_canonizacao
			$array = array();

			foreach ((array) $data->etapa_canonizacao as $value)
			{
				if (!is_array($value))
				{
					$array[] = $value;
			...
			$data->etapa_canonizacao = $array;
			}

			// Support for multiple or not foreign key field: categoria
			$array = array();

			foreach ((array) $data->categoria as $value)
			{
				if (!is_array($value))
				{
					$array[] = $value;
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_acervosalesiano'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__acervosalesiano');
				$max             = $db->loadResult();
