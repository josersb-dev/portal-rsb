...
class ItemModel extends ItemModel
{
	...
	{
		$app  = Factory::getApplication('com_noticias');
		$user = $app->getIdentity();

		// Check published state
		if ((!$user->authorise('core.edit.state', 'com_noticias')) && (!$user->authorise('core.edit', 'com_noticias')))
		{
		...
		{
			$id = Factory::getApplication()->getUserState('com_noticias.edit.item.id');
		}
		else
		{
			$id = Factory::getApplication()->input->get('id');
			Factory::getApplication()->setUserState('com_noticias.edit.item.id', $id);
		}
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_NOTICIAS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_NOTICIAS_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->id_segmento) && $this->_item->id_segmento != '')
		{
			if (is_object($this->_item->id_segmento))
			{
				$this->_item->id_segmento = ArrayHelper::fromObject($this->_item->id_segmento);
			}

			if (is_array($this->_item->id_segmento))
			{
				$this->_item->id_segmento = implode(',', $this->_item->id_segmento);
			}
			...
			$query
				...
				->where('FIND_IN_SET(' . $db->quoteName('id') . ', ' . $db->quote($this->_item->id_segmento) . ')');

			$db->setQuery($query);

			$result = $db->loadColumn();

			$this->_item->id_segmento = !empty($result) ? implode(', ', $result) : '';
		}
