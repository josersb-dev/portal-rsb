 ...
 * @since 1.0.0
 */
class SppagebuilderAddonVideohome extends SppagebuilderAddons
{
	/**
			...
			foreach ($settings->sp_carouselpro_item as $key => $value) {
				...
				if ((isset($value->title) && $value->title) || (isset($value->content) && $value->content)) {
					...
					if (isset($value->button_text) && $value->button_text) {
						...
						$button_class .= (isset($settings->button_block) && $settings->button_block) ? ' ' . $value->button_block : '';
				...
				/*if ($video) {
					...
					$output  .= ($item_image_src) ? '<a class="sppb-modal-selector sppb-magnific-popup" data-popup_type="iframe" data-allow="autoplay" data-mainclass="mfp-no-margins mfp-with-zoom" href="'.$videoUrl.'" id="sppb-modal-1691422172233-selector"><img class="sppb-img-reponsive" src="' . $item_image_src . '" alt="' . (isset($value->title) ? $value->title : '') . '"></a>' : '';
				//}
