 ...
 * @since 1.0.0
 */
class SppagebuilderAddonRSB_noticias extends SppagebuilderAddons
{
	/**
	...
	public function render()
	{
		...
		$segmento = is_array($item->segmento) ? $item->segmento[0] : [];
		$linkVerTodas = $item->linkvertodas ? $item->linkvertodas : JRoute::_('index.php?option=com_content&view=category&layout=blog&id='.$segmento);

		$output = '
		<div id="rsb-noticias">
			<div class="rsb-not-title">
				<h4>Notícias</h4>
			</div>
			<div class="owl-carousel owl-theme rsb-noticias owl-loaded owl-drag" id="rsb-segmentos">
			...
			<a href="'.$linkVerTodas.'" class="btn btn-azul">Ver Todas</a>
		</div>
	...
	public function stylesheets() {
		return array(
			JURI::base(true) . '/components/com_sppagebuilder/addons/rsb_noticias/assets/css/style.css?'.uniqid(),
...
return $js;
    ...
    public static function getItems($item)
    {
		...
		$segmento = is_array($item->segmento) ? $item->segmento : [];
		$segmento = array_map(function($item){ return intval($item);},$segmento);
		...
		if(count($item->segmento)){
			$query->where($db->quoteName('catid') . ' IN (' . implode(',',$segmento) .')');
		}
        ...
        foreach($results as $item){
            ...
            <div class="owl-item" style="display:none">
				<a href="'. JRoute::_(ContentHelperRoute::getArticleRoute($item->id, $item->catid, $item->language)).'">
					<div class="rsb-not-img">
						<img class="owl-lazy" data-src="'.json_decode($item->images)->image_intro.'" />
					</div>
					<div class="rsb-not-cont">
						<span class="rsb-not-data text-muted">'.$dataItem.'</span>
						<h6>'.$item->title.'</h6>
						<p class="rsb-not-text">'.self::limitarTexto($item->introtext,200).'</p>
					</div>
				</a>
            </div>
            ';
